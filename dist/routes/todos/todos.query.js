...
async function get_last_todo() {
    return new Promise((resolve, reject) => {
        db_1.default.query('SELECT * FROM todo ORDER BY id DESC LIMIT 1', function (err, result, fields) {
            if (err)
                reject(err);
            resolve(result);
        });
...
async function post_todo(title, description, due_time, user_id, status) {
    return new Promise((resolve, reject) => {
        db_1.default.execute('INSERT INTO todo (title, description, due_time, user_id, status) VALUES("' + title + '", "' + description + '", "' + due_time + '", "' + user_id + '", "' + status + '")', async function (err, result, fields) {
            if (err)
                reject(err);
            try {
...
async function update_todo_information(title, description, due_time, user_id, status, id) {
    return new Promise((resolve, rejects) => {
        db_1.default.execute('UPDATE `todo` SET `title` = ?, `description` = ?, `due_time` = ?, `user_id` = ?, `status` = ? WHERE `id` = ?', [title, description, due_time, user_id, status, id], function (err, result, fields) {
            db_1.default.execute('SELECT title, description, due_time, user_id, status FROM todo WHERE id = ?', [id], function (err, result, fields) {
                if (err)
                    rejects(err);
                resolve(result);
            });
...
async function delete_todo(res, id) {
    return new Promise((resolve, rejects) => {
        db_1.default.execute('DELETE FROM todo WHERE id = ?', [id], function (err, result, fields) {
            if (err)
                rejects(err);
            resolve(result);
        });
