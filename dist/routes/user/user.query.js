...
exports.get_user_task = exports.update_user_information = exports.delete_user = exports.information_email = exports.information_id = exports.get_all_user_task = exports.get_all_user_info = void 0;
...
;
async function get_all_user_task(res, id) {
    return new Promise((resolve, reject) => {
        db_1.default.query('SELECT * FROM todo WHERE user_id = ?', [id], function (err, result) {
            if (err)
                reject(err);
            resolve(result);
        });
...
exports.get_all_user_task = get_all_user_task;
;
...
async function delete_user(res, id) {
    return new Promise((resolve, rejects) => {
        db_1.default.execute('DELETE FROM user WHERE id = ?', [id], function (err, result, fields) {
...
async function update_user_information(res, password, id, email, name, firstname) {
    return new Promise((resolve, rejects) => {
        db_1.default.execute('UPDATE `user` SET `email` = ?, `password` = ?, `name` = ?, `firstname` = ? WHERE `id` = ?', [email, password, name, firstname, id], function (err, result, fields) {
            if (err)
                rejects(err);
            db_1.default.execute('SELECT id, email, password, name, firstname, created_at FROM user WHERE id = ?', [id], function (err, result, fields) {
                if (err)
                    rejects(err);
                resolve(result);
            });
